            ...
            <div v-if="todoTab.length" class="flex space-x-2">
              <div class="w-1/2">
                ...
                <ul v-if="todoOp(todoTab, false).length" class="list-container">
                  <li
                    v-for="todo in todoOp(todoTab, false)"
                    class="list-content"
                  >
                    <span> {{ todo.title }}</span>
                    <div class="flex flex-col">
                      <span class="text-right">{{
                        dayjs(todo.created_at).format("DD-MM-YYYY HH:mm")
                      }}</span>
                      <div class="space-x-2">
                        <span
              ...
              <div class="w-1/2">
                ...
                <ul v-if="todoOp(todoTab, true).length" class="list-container">
                  <li
                    v-for="todo in todoOp(todoTab, true)"
                    class="list-content"
                  >
                    <span> {{ todo.title }}</span>
                    <div class="flex flex-col">
                      <span class="text-right">{{
                        dayjs(todo.updated_at).format("DD-MM-YYYY HH:mm")
                      }}</span>
                      <div class="space-x-2">
                        <span
                          class="btn bg-red-300 text-red-500"
...
export default {
  components: { ValidationError, UserLayout },
  setup(props) {
